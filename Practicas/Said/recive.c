...
unsigned char ip[4];
unsigned char trama_e[1514], trama_r[1514];
unsigned char ETHERTYPE[2] = {0x0a,0x0a};
void print_t(unsigned char *, int);
...
void recive(int ds, unsigned char *trama){
	int size = recvfrom(ds, trama, 1514,0,NULL,0);
	if (size == -1){
		printf("Error :(\n");
	}else{
		print_t(trama, size);
...
void print_t(unsigned char *trama, int size){
